...
7. If the profile has a P&L ledger configured (see `../references/pnl-ledger.md`), also capture the stake, the combined decimal price, and per-leg odds when visible — these go only into the per-job `LEDGER` config, never into the tracking prompt. If stake or combined price is unknown, skip the ledger for this slip and say so.
8. Show a parsed preview and ask the user to correct errors before tracking.
