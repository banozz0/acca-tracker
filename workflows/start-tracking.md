...
2. Attach a per-job score script: copy `scripts/fetch-scores.py` to `~/.hermes/scripts/acca-<id>.py`, fill in its `SLIP` and `RUN_DATES`, and create the job with `--script acca-<id>.py`. The prompt reads the injected `LIVE SCORES:` block instead of fetching — this is what keeps scores reliable (see SKILL.md "Scheduled agents are time-blind and fetch unreliably").
...
7. Explain that reports are status-only and may mark legs `UNVERIFIABLE` when data is missing or ambiguous.
